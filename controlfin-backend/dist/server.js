...
fastify.register(rate_limit_1.default, {
    ...
    timeWindow: `${Math.max(env_1.env.rateLimitWindowMs, 1000)} ms`,
});
...
const start = async () => {
    try {
        ...
        fastify.log.info(`🚀 Server listening on http://0.0.0.0:${env_1.env.port}`);
        fastify.log.info(`📚 API Documentation available at http://0.0.0.0:${env_1.env.port}/api/docs`);
    }
    catch (err) {
